...
type PaymentProfile struct {
	ID                    int64       `json:"id,omitempty"`                       // the id after the profile is created
	...
	ExpirationYear        string      `json:"expiration_year,omitempty"`          // 	(Required when payment_type is credit_card unless you provide the vault_token) The 4-digit credit card expiration year, as an integer or string, i.e. 2012
	CVV                   string      `json:"cvv,omitempty"`                      // 	(Optional, may be required by your gateway settings) The 3- or 4-digit Card Verification Value. This value is merely passed through to the payment gateway.
	BillingAddress        string      `json:"billing_address,omitempty"`          // 	(Optional, may be required by your product configuration or gateway settings) The credit card or bank account billing street address (i.e. 123 Main St.). This value is merely passed through to the payment gateway.
	BillingAddress2       string      `json:"billing_address_2,omitempty"`        // 	(Optional) Second line of the customer’s billing address i.e. Apt. 100
	BillingCity           string      `json:"billing_city,omitempty"`             // 	(Optional, may be required by your product configuration or gateway settings) The credit card or bank account billing address city (i.e. Boston). This value is merely passed through to the payment gateway.
	BillingState          string      `json:"billing_state,omitempty"`            // 	(Optional, may be required by your product configuration or gateway settings) The credit card or bank account billing address state (i.e. MA). This value is merely passed through to the payment gateway.
	BillingZip            string      `json:"billing_zip,omitempty"`              // 	(Optional, may be required by your product configuration or gateway settings) The credit card or bank account billing address zip code (i.e. “12345”). This value is merely passed through to the payment gateway.
	...
	BankName              string      `json:"bank_name,omitempty"`                // 	(Required when creating a subscription with ACH) The name of the bank where the customer’s account resides
	BankRouting           string      `json:"bank_routing_number,omitempty"`      // 	(Required when creating a subscription with ACH) The routing number of the bank
	BankAccount           string      `json:"	bank_account_number,omitempty"`     // 	Required when creating a subscription with ACH) The customer’s bank account number
	BankAccountType       string      `json:"	bank_account_type,omitempty"`       // 	When payment_type is bank_account, this defaults to checking and cannot be changed
	BankAccountHolderType string      `json:"bank_account_holder_type,omitempty"` // 	When payment_type is bank_account, may be personal (default) or business
	Verified              bool        `json:"verified,omitempty"`                 // 	When payment type is bank_account and current_vault is stripe_connect, may be set to true to indicate that the bank account has already been verified.
	PaypalEmail           string      `json:"paypal_email,omitempty"`             //
	PaymentMethodNonce    string      `json:"payment_method_nonce,omitempty"`     //
	VaultToken            string      `json:"vault_token,omitempty"`              // 	(Only allowed during the creation of a new payment profile.) If you have an existing vault_token from your gateway, you may associate it with this new payment profile.
	ChargifyToken         string      `json:"chargify_token,omitempty"`           // 	(Optional) Token received after sending billing informations using . This token must be passed along with customer_id attribute (i.e. tok_9g6hw85pnpt6knmskpwp4ttt)
	CurrentVault          VaultMethod `json:"current_vault,omitempty"`            // 	(Required when you pass in a vault_token.) Will be one of the following: bogus (for testing), authorizenet, authorizenet_cim, beanstream, bpoint, braintree_blue, chargify, cybersource, elavon, eway, eway_rapid_std , firstdata, fusebox, litle, moneris, moneris_us, orbital, payment_express, paymill, pin, quickpay, square, stripe_connect, trust_commerce, wirecard. Provides a hint about where the credit card details represented by vault_token are stored, however transactions will always be sent to the gateway configured in the Site's settings.
	CardType              string      `json:"card_type,omitempty"`                // 	Can be any of the following visa, master, discover, american_express, diners_club, jcb, switch, solo, dankort, maestro, forbrugsforeningen, laser
}
